...
외부 cron 서비스, 30분마다
  ...
  -> 새 슬롯이 없어도 시간대별 예약 가능 날짜 목록을 포함한 정상 크롤링 상태 알림
...
- 정상 크롤링도 Telegram으로 알려주도록 GitHub Actions에서 `TELEGRAM_NOTIFY_STATUS=1`을 설정한다. 이 메시지에는 `06:00~08:00: 07/01(수), ...`처럼 시간대별 예약 가능 날짜 목록이 포함된다.
